	...
	for enemy in $Objects/Enemies.get_children():
		...
		for item in enemy.contents:
			if item[0] == 0:
				...
				pass
	#----------
	player = preload("res://src/player.tscn").instantiate()
	...
	for type in ["ladder", "death_tile"]:
		...
		match type:
			"ladder":
				...
				resource_string = "res://src/objects/generic/ladder.tscn" 
			...
			"death_tile":
				...
				resource_string = "res://src/objects/generic/death_tile.tscn"
