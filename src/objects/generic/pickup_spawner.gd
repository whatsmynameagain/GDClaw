...
func spawn_elements() -> void:
	for elem in spawn_list:
		...
		match elem[0]:
			0:
				item = preload("res://src/objects/generic/treasure.tscn").instantiate()
				item.type = elem[1]
				item.color = elem[2] if not (item.type in ["Coin", "Bars", "Pearls"]) else "None"
			1:
				item = preload("res://src/objects/generic/restore.tscn").instantiate()
				item.type = elem[1]
			...
			2:
				item = preload("res://src/objects/generic/powerup.tscn").instantiate()
				item.type = elem[1]
				item.duration = elem[2]
				item.stack_duration = elem[3]
				item.one_use = elem[4]
			3:
				item = preload("res://src/objects/generic/teleporter.tscn").instantiate()
				item.type = elem[1]
				item.destination = elem[2]
				item.orientation = elem[3]
				item.one_use = elem[4]
			4:
				item = preload("res://src/objects/generic/end_item.tscn").instantiate()
				item.type = elem[1]
